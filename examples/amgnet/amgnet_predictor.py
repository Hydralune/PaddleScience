...
class AMGNPredictor(Predictor):
        ...
        cfg = OmegaConf.create(
            {
                ...
                "INFER": {
                    ...
                    "enable_mkldnn": config_params.get("use_mkldnn", True)
                    if config_params
                    else True,
                    "enable_memory_optim": False,  # Disable to avoid compatibility issues
                    "enable_ir_optim": config_params.get("ir_optim", True)
                    if config_params
                    else True,
                    "cpu_threads": 4,
                },
    ...
    def predict(
        self, inputs: Dict[str, np.ndarray], batch_size: int = 1
    ) -> Dict[str, np.ndarray]:
        ...
        logger.message(
            f"Processing speed: {node_count / inference_time:.2f} nodes/second"
        )
    ...
    def _plot_error_histogram(self, error_data: np.ndarray, title: str, bins: int = 50):
        ...
        plt.savefig(
            f"./result/analysis/{title.lower().replace(' ', '_')}_histogram.png"
        )
        plt.close()
    ...
    def visualize_comparison(
        ...
        for i in range(prediction.shape[1]):
            ...
            plt.subplot(1, 3, 1)
            sc = plt.scatter(
                coords[:, 0], coords[:, 1], c=ground_truth[:, i], cmap="viridis", s=5
            )
            plt.colorbar(sc)
            ...
            plt.subplot(1, 3, 2)
            sc = plt.scatter(
                coords[:, 0], coords[:, 1], c=prediction[:, i], cmap="viridis", s=5
            )
            plt.colorbar(sc)
        ...
        logger.message("Visualizations saved to ./result/comparison")
